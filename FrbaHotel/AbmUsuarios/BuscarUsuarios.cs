using System;
using System.Collections.Generic;
using System.Data;
using System.Linq;
using System.Text;

namespace FrbaHotel.AbmUsuarios
{
    class BuscarUsuarios
    {
        internal DataTable Buscar(ConexionSQL c, string ape, string dir, string mail, string nom, string numdoc, string tel, string tipodoc, string usu, DateTime naci, string hotelid)
        {
        int hayfiltro=0;
            string query= "SELECT * from WHERE_EN_EL_DELETE_FROM.empleados ";
            if (hotelid.Equals("")){}else{
                query+= " join WHERE_EN_EL_DELETE_FROM.empleados_hoteles on empleados.empleado_id = empleados_hoteles.empleado_id"
                    + " where empleados_hoteles.hotel_id=" + hotelid;
                hayfiltro=1;
            }
           
            if(ape.Equals("")){}else{
                hayfiltro=1; query+= "where apellido like'%" + ape +"%'"; 
            };
            if(dir.Equals("")){}else{
                if(hayfiltro.Equals(1))
                {
                    query+= " and direccion like '%" + dir + "%'";
                }
                else{
                    query+= " where direccion like '%" + dir + "%'"; hayfiltro=1;
                }
            };
            if(mail.Equals("")){}else{
                if(hayfiltro.Equals(1)){
                    query+= " and mail like '%" + mail + "%'";
                }
                else{
                    query+= " where mail like '%" + mail + "%'"; hayfiltro=1;
                }
            };
            if (nom.Equals("")){}else{
                if (hayfiltro.Equals(1)){
                    query+= " and nombre like '%" + nom + "%'";
                }else
                {
                    query+= " where nombre like '%" + nom + "%'";
                    hayfiltro=1;
                }
            }
            if (numdoc.Equals("")){}else
            {
                if (hayfiltro.Equals(1)){
                    query+= " and documento_nro like '%" + numdoc + "%'";
                }else{
                    query+= "where documento_nro like '%" + numdoc + "%'";
                    hayfiltro=1;
                }
            }
            if (tel.Equals("")){}else
            {
                if (hayfiltro.Equals(1)){
                    query+= " and telefono like '%" + tel + "%'";
                }else{
                    query+= " where telefono like '%" + tel + "%'";
                    hayfiltro=1;
                }
            }
            if (tipodoc.Equals("")){}else{
                if (hayfiltro.Equals(1)){
                    query+= " and documento_tipo like '%" + tipodoc + "%'";
                    }else{
                    query+= " where documento_tipo like '%" + tipodoc + "%'";
                    hayfiltro=1;
                }
            }
            if (usu.Equals("")){}else{
                if (hayfiltro.Equals(1)){
                    query+= " and usuario like '%" + usu + "%'";
                }else{
                    query+= " where usuario like '%" + usu + "%'";
                    hayfiltro=1;
                }
            }

            return c.cargarTablaSQL(query);

        }

        internal DataTable BusquedaInicial(ConexionSQL c)
        {
            return c.cargarTablaSQL("select e.*, isnull(ho.nombre, 'Hotel ' + direccion) as hotel_nombre from WHERE_EN_EL_DELETE_FROM.empleados e join "
                + " WHERE_EN_EL_DELETE_FROM.empleados_hoteles eh on e.empleado_id=eh.empleado_id join WHERE_EN_EL_DELETE_FROM.hoteles ho on eh.hotel_id=ho.hotel_id");

        }

        internal DataTable GetHoteles()
        {
            ConexionSQL c = new ConexionSQL();
           return  c.cargarTablaSQL("select hotel_id, isnull(nombre, 'Hotel ' + direccion) as nombre from WHERE_EN_EL_DELETE_FROM.hoteles");
        }
    }
}
